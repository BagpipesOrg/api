...
export function validateDiagramData(diagramData: any): any {
  //DiagramData
  ...
  const startingNodes = diagramData.nodes.filter((node) => !diagramData.edges.find((edge) => edge.target === node.id))
  ...
  const endingNodes = diagramData.nodes.filter((node) => !diagramData.edges.find((edge) => edge.source === node.id))
